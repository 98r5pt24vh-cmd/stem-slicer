...
class MidiEngineSignals(QObject):
    ready = Signal()
    failed = Signal(str)
...
class MidiEngineService(threading.Thread):
    """Own the Basic Pitch session for its complete lifetime.

    ONNX construction, warm-up and inference deliberately stay on this one
    native Python thread.  This removes the former cross-thread ownership
    handoff and gives the converter one deterministic execution context.
    """
    ...
    def submit(self, layers, cache_path, job_id):
        self.jobs.put((list(layers), str(cache_path), int(job_id)))
    ...
    def _convert(self, converter, layers, cache_path, job_id):
        started = time.perf_counter()
        ready_count = 0
        total = len(layers)
        diagnostics = get_diagnostics()
        diagnostics.event("midi_job_started", job_id=job_id, layers=total)
        for index, layer in enumerate(layers, 1):
            if self._stop_requested.is_set() or job_id != self.latest_job_id:
                break
            path = layer["path"]
            midi_path = os.path.join(cache_path, f"layer_{index}.mid")
            try:
                converter.convert(path, midi_path, bpm=int(layer.get("bpm") or 140))
                if self._stop_requested.is_set() or job_id != self.latest_job_id:
                    break
                ready_count += 1
                if ready_count == 1:
                    diagnostics.event(
                        "midi_first_file_ready",
                        job_id=job_id,
                        duration_seconds=time.perf_counter() - started,
                    )
    ...
    def run(self):
        started = time.perf_counter()
        diagnostics = get_diagnostics()
        diagnostics.event("midi_engine_load_started")
        try:
            from midi_conversion import MidiConverter

            diagnostics.event(
                "midi_engine_import_complete",
                duration_seconds=time.perf_counter() - started,
            )
        ...
        while not self._stop_requested.is_set():
            ...
            if job is None:
                break
            layers, cache_path, job_id = job
            if job_id != self.latest_job_id:
                continue
            self._convert(converter, layers, cache_path, job_id)
...
class WaveformWidget(QWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton and self.width() > 0:
            self.scrubbing = True; self._seek_from_x(event.position().x()); event.accept(); return
        super().mousePressEvent(event)

    def mouseMoveEvent(self, event):
        if self.scrubbing and (event.buttons() & Qt.LeftButton):
            self._seek_from_x(event.position().x()); event.accept(); return
        super().mouseMoveEvent(event)

    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton and self.scrubbing:
            self._seek_from_x(event.position().x()); self.scrubbing = False; event.accept(); return
...
class LineIcon(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.kind in ("folder", "folder_in", "folder_out"):
            ...
            # Five-pip die: generation is a random selection, not a retarget
            # operation.  Keep the same single-stroke visual language as the
            # other navigation icons.
            p.drawRoundedRect(QRectF(.15*w, .15*h, .70*w, .70*h), .12*w, .12*w)
            p.setPen(Qt.NoPen)
            p.setBrush(self.color)
            radius = .055*w
            for x, y in ((.32, .32), (.68, .32), (.50, .50), (.32, .68), (.68, .68)):
                p.drawEllipse(QRectF(x*w-radius, y*h-radius, 2*radius, 2*radius))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.midiRequested.connect(self._submit_midi_job)
        self.layer_cards = []
    ...
    def _start_midi_engine(self):
        ...
        get_diagnostics().event("midi_engine_start_requested")
    ...
    def _submit_midi_job(self, layers, cache_path, job_id):
        if self.midi_service is not None:
            self.midi_service.submit(layers, cache_path, job_id)
    ...
    def _midi_engine_ready(self):
        ...
        get_diagnostics().event("midi_engine_ready")
