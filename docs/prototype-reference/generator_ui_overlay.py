"""Development overlay for the Stem Slicer 1.9 Generate cards-as-slots UI.

Only the Generate interface lives in this folder.  Engines, assets and the
controller continue to come from the canonical 1.9 source so visual iterations
do not require rebuilding the macOS application.
"""
...
_BASE_NAME = "_stem_slicer_19_generator_ui_base"
_SPEC = importlib.util.spec_from_file_location(
    _BASE_NAME,
    SOURCE_ROOT / "generator_ui.py",
)
if _SPEC is None or _SPEC.loader is None:
    raise RuntimeError("Unable to load the Stem Slicer 1.9 Generate interface")
