...
class CollaboratorGenerationTests(unittest.TestCase):
    ...
    def test_local_aliases_merge_into_the_canonical_cloud_identity(self) -> None:
        identities = bridge._producer_identity_signature([
            {"canonicalName": "XT", "aliases": ["Tinex", "T-Next Is Here"]},
        ])
        records = [
            {"filename": "Tinex 140 Tinex_L1.mp3"},
            {"filename": "NIGHT DRIVE 140 T-Next Is Here_L1.mp3"},
            {"filename": "PARIS 123 C#MIN @Tinex_L1.mp3"},
        ]

        self.assertEqual(
            bridge._source_provenance(records[0]["filename"], "+NRGY", identities),
            {"loop_name": "Tinex", "producers": ["XT"]},
        )
        self.assertEqual(
            bridge._filter_records_by_allowed_producers(
                records,
                allowed_producers=["+NRGY", "XT"],
                producer_identities=identities,
            ),
            records,
        )

    def test_musical_key_variants_never_become_producers(self) -> None:
        variants = ("C#MIN", "c#min", "C#m", "C# MIN", "c# minor", "D♭MAJ", "Db major", "E", "F#", "g♭")

        for key in variants:
            with self.subTest(key=key):
                record = {"filename": f"LOOP 140 XT {key}_L1.mp3"}
                self.assertEqual(bridge._record_producers(record), ["XT"])

    def test_signed_in_producer_is_the_generation_owner(self) -> None:
        record = {"filename": "FMIN PRIVATE LOOP 140_L1.mp3"}
        candidate = self.candidate("Bass", None, 1)

        self.assertEqual(bridge._record_producers(record, "XT"), ["XT"])
        self.assertEqual(
            bridge._generation_producers(
                [SimpleNamespace(candidate=candidate)],
                {candidate.identity: ["XT"]},
                "XT",
            ),
            ["XT"],
        )
...
class CloudLayerMaterializationTests(unittest.TestCase):
    def test_generation_manifest_records_phase_timings(self) -> None:
        with tempfile.TemporaryDirectory() as root:
            manifest_path = Path(root) / "generation.json"
            manifest_path.write_text('{"schema_version": 3}\n', encoding="utf-8")

            bridge._annotate_generation_manifest_performance(
                manifest_path,
                5.0,
                {"cloudSeconds": 1.25, "renderSeconds": 3.5},
            )

            performance = json.loads(manifest_path.read_text(encoding="utf-8"))["performance"]
            self.assertEqual(performance["total_seconds"], 5.0)
            self.assertEqual(performance["cloud_seconds"], 1.25)
            self.assertEqual(performance["render_seconds"], 3.5)
        ...
        try:
            with tempfile.TemporaryDirectory() as root:
                ...
                with patch.object(bridge, "progress_percent"):
                    with patch.object(bridge, "_sha256_file", wraps=bridge._sha256_file) as checksum:
