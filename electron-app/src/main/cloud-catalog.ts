...
export function readLocalCloudManifest(
  ...
  try {
    ...
    const optionalColumn = (name: string, fallback: string): string => (
      columns.has(name) ? name : `${fallback} AS ${name}`
    )
    const numericColumn = (name: string): string => (
      columns.has(name) ? `COALESCE(${name}, 0) AS ${name}` : `0 AS ${name}`
    )
    const activeRows = columns.has("manual_excluded")
    ...
    rows = database.prepare(`
      SELECT
        ...
        ${optionalColumn("manual_bpm", "NULL")},
        ${optionalColumn("manual_key", "NULL")},
        ${optionalColumn("manual_mode", "NULL")},
        ${numericColumn("timeline_offset_beats")},
        ${numericColumn("trim_start_beats")},
        ${numericColumn("trim_end_beats")}
      FROM layer_cache
      WHERE library_root = ? ${activeRows}
