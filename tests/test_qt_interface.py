...
class QtInterfaceTests(unittest.TestCase):
    ...
    def test_main_pages_are_stable_and_navigable(self):
        ...
        original_pages = tuple(window.pages.widget(index) for index in range(3))
        ...
        QTest.mouseClick(window.generate_tab, Qt.LeftButton)
        APP.processEvents()
        ...
        APP.processEvents()
        self.assertEqual(window.pages.currentIndex(), 0)
        self.assertEqual(original_pages, tuple(window.pages.widget(index) for index in range(3)))
        window.close()
    ...
    def test_quick_target_fields_are_readable_and_never_clipped(self):
        ...
        for selector in (window.quick_extract_key, window.quick_convert_key):
            ...
            was_enabled = selector.isEnabled()
            ...
            self.assertEqual(selector._popup.width(), visible_width)
            selector._popup.hide()
            selector.setEnabled(was_enabled)
        window.select_tab(0)
        APP.processEvents()
    ...
    def test_scale_popup_fits_its_rows_and_current_screen_at_every_scale(self):
        ...
        for percent in (100, 110, 120, 130, 140, 150):
            ...
            self.assertEqual(popup.width(), visible_width)
    ...
    def test_target_key_popups_stay_inside_the_available_screen(self):
        ...
        for percent in (100, 110, 120, 130, 140, 150):
            ...
            self.assertGreaterEqual(popup.height(), popup.sizeHint().height())
            visible_width = abs(
                window.target_key_combo.mapToGlobal(
                    QPoint(window.target_key_combo.width(), 0)
                ).x()
                - window.target_key_combo.mapToGlobal(QPoint(0, 0)).x()
            )
            self.assertEqual(popup.width(), visible_width)
            popup.hide()
        window.close()
    ...
    def test_pending_midi_job_starts_once_when_engine_becomes_ready(self):
        window = MainWindow()
        layer = {
            "path": "/private/tmp/pending-midi-layer.mp3",
            "name": "pending-midi-layer.mp3",
            "bpm": 140,
            "duration": 1.0,
            "bytes": 32,
            "peaks": [0.0] * 72,
        }
        window.midi_engine_state = "loading"
        window.midi_worker = type("Worker", (), {"latest_job_id": 0})()
        requests = []
        window.midiRequested.connect(lambda *args: requests.append(args))
        ...
        self.assertIsNotNone(window.pending_midi_job)

        window._midi_engine_ready()
        self.assertEqual(window.midi_engine_state, "ready")
        self.assertIsNone(window.pending_midi_job)
        self.assertEqual(len(requests), 1)
        self.assertEqual(requests[0][0][0]["path"], layer["path"])
        self.assertEqual(requests[0][2], window.midi_job_id)
        window.close()
    ...
    def test_waveform_scrub_survives_the_graphics_proxy_drag(self):
        ...
        view.resize(360, 100)
        waveform = WaveformWidget([0.5] * 72)
        waveform.resize(240, 22)
        proxy = scene.addWidget(waveform)
        proxy.setPos(20, 20)
        view.show()
        APP.processEvents()

        values = []
        waveform.seekRequested.connect(values.append)
        start = view.mapFromScene(proxy.mapToScene(QPointF(3, 11)))
        end = view.mapFromScene(proxy.mapToScene(QPointF(237, 11)))
        QTest.mousePress(view.viewport(), Qt.LeftButton, Qt.NoModifier, start)
        QTest.mouseMove(view.viewport(), end, 20)
        QTest.mouseRelease(
            view.viewport(), Qt.LeftButton, Qt.NoModifier, end
        )
        APP.processEvents()

        self.assertGreaterEqual(len(values), 2)
        self.assertLess(values[0], 0.05)
