"""Native PySide6 port of the validated Stem Slicer interface.
...
def validated_stylesheet() -> str:
    ...
    * {{
    ...
    QFrame[role="layerCard"][locked="true"] {{ background: #182019; border: 1px solid #57d84e; }}
    QPushButton[role="cardLock"] {{
        background: #10171a; border: 1px solid #3b4850; border-radius: 5px;
        color: #849098; padding: 0; font-size: 7px; font-weight: 900;
    }}
    QPushButton[role="cardLock"]:checked {{ background: #17331a; border-color: #57d84e; color: #83f07b; }}
    QPushButton[role="cardAltKey"] {{
        background: #181321; border: 1px solid #7653a8; border-radius: 5px;
        color: #caa9eb; padding: 0; font-size: 7px; font-weight: 900;
    }}
    QPushButton[role="cardAltKey"]:hover {{ background: #241a32; border-color: #b58ae5; }}
    QPushButton[role="cardAltKey"]:disabled {{ background: #111619; border-color: #303a40; color: #667179; }}
    QPushButton[role="cardAltKey"][state="busy"] {{ border-color: #7653a8; color: #b58ae5; }}
    QPushButton[role="cardAltKey"][state="alternate"],
    QPushButton[role="cardAltKey"][state="active"] {{ background: #241a32; border-color: #9b6bcf; color: #d6b7f5; }}
    QFrame[role="recipeSlot"] {{
        background: #0d151a; border: 1px solid #2b3b44; border-radius: 5px;
    }}
    QFrame[role="recipeSlot"][locked="true"] {{
        background: #101518; border-color: #3b464d;
    }}
    QFrame[role="recipeSlot"] QPushButton[role="choiceSelector"] {{
        background: transparent; border: none; border-radius: 0;
    }}
    QFrame[role="recipeSlot"] QPushButton[role="choiceSelector"]:hover {{
        background: #151e23;
    }}
    QPushButton[role="slotRemove"] {{
        background: #201416; border: none;
        border-top-left-radius: 4px; border-bottom-left-radius: 4px;
        color: transparent; padding: 0;
    }}
    QPushButton[role="slotRemove"]:hover {{ background: #35191b; }}
    QPushButton[role="slotRemove"]:pressed {{ background: #160d0e; }}
    QPushButton[role="slotRemove"]:disabled {{ background: #101518; }}
    QPushButton[role="slotAdd"] {{
        background: #102116; border: 1px solid #3b7a42; border-radius: 5px;
        color: #57d84e; padding: 0;
    }}
    QPushButton[role="slotAdd"]:hover {{ background: #17331a; border-color: #57d84e; }}
    QPushButton[role="slotAdd"]:disabled {{ background: #0c1114; border-color: #202a30; color: #475159; }}
    QScrollArea[role="slotStrip"], QScrollArea[role="slotStrip"] > QWidget > QWidget {{ background: transparent; border: none; }}
    ...
    QScrollBar:horizontal {{ background: #0d1012; height: 6px; margin: 1px 3px; }}
    QScrollBar::handle:horizontal {{ background: #454c53; border-radius: 3px; min-width: 24px; }}
...
class PopupChoiceRow(QFrame):
    ...
    def __init__(self, text: str, accent: str, *, show_check=True, parent=None):
        super().__init__(parent)
        self.value = str(text)
        self.accent = accent
        self._show_check = bool(show_check)
        self._checked = False
        self._hovered = False
        self.setCursor(Qt.PointingHandCursor)
        ...
        self.check_label.setFixedWidth(10 if self._show_check else 0)
        self.text_label = QLabel(self.value)
        self.text_label.setAlignment(
            Qt.AlignVCenter | (Qt.AlignLeft if self._show_check else Qt.AlignHCenter)
        )
    ...
    def setChecked(self, checked: bool) -> None:
        self._checked = bool(checked)
        self.check_label.setText("✓" if self._checked and self._show_check else "")
        self._refresh_style()

    def setVisualScale(self, factor: float) -> None:
        factor = max(1.0, float(factor))
        self.setFixedHeight(round(25 * factor))
        self.layout().setContentsMargins(round(6 * factor), 0, round(6 * factor), 0)
        self.layout().setSpacing(round(4 * factor))
        self.check_label.setFixedWidth(round(10 * factor) if self._show_check else 0)
        font = self.font()
...
class AnchoredChoiceSelector(QPushButton):
    ...
    def _popup_size(self):
        ...
        if not self._exact_popup_width:
...
class TargetKeySelector(AnchoredChoiceSelector):
    ...
    def setCompactWidth(self, width):
        ...
        # The application QSS owns the font size, so QFont.setPixelSize() is
        # overridden during the Windows polish pass.  A per-widget QSS value
        # has higher precedence and remains measurable after that pass.
...
class ValidatedMainWindow(FunctionalMainWindow):
    ...
    def _build(self):
        ...
        # Import lazily to avoid a module cycle: Generate reuses the validated
        # selectors, tabs and theme from this module.  At construction time
        # the module is fully initialized, so the production page can be
        # embedded as the genuine third surface without touching the two
        # accepted workflows.
        from generator_ui import GeneratePage

        self.pages = QStackedWidget()
        self.pages.addWidget(self._stem_page())
        self.pages.addWidget(self._quick_page())
        self.generate_page = GeneratePage(self.pages)
    ...
    def _header(self, outer):
        ...
        bc = QVBoxLayout(); bc.setSpacing(2); title = QLabel("LOOP LAYER EXTRACTION SYSTEM"); version = QLabel(APP_VERSION)
    ...
    def _tabs(self, outer):
        ...
        self.generate_tab = V16Tab("dice", "GENERATE"); self.generate_tab.setFixedWidth(210)
        self.stem_tab.clicked.connect(lambda: self.select_tab(0)); self.quick_tab.clicked.connect(lambda: self.select_tab(1)); self.generate_tab.clicked.connect(lambda: self.select_tab(2))
        row.addWidget(self.stem_tab); row.addWidget(self.quick_tab); row.addWidget(self.generate_tab); row.addStretch(); outer.addWidget(tabs)
    ...
    def _populate_layer_cards(self, layers, empty_text=None, show_empty_icon=False):
        ...
        if layers:
            ...
            # long enough for macOS to report the app as unresponsive.  Render
            # one visual row per event-loop turn instead.  The audio result is
            # already complete; this only makes its presentation cooperative.
    ...
    def select_tab(self, index):
        self.pages.setCurrentIndex(index); self.stem_tab.setActive(index == 0); self.quick_tab.setActive(index == 1); self.generate_tab.setActive(index == 2)
