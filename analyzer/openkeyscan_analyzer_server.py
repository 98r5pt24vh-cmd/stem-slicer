...
try:
    ...
    scipy_loaded = True
    print("Scipy audio modules pre-loaded successfully", file=sys.stderr)

except Exception as e:
    print(f"Warning: Scipy initialization error: {e}", file=sys.stderr)

# Numba stores Librosa's frozen source paths as package-relative names. Keep the
# analyzer itself independent from its launcher by resolving those names from
# PyInstaller's payload root.
...
from loop_bpm import (
    AUDIO_SECONDS as LOOP_BPM_AUDIO_SECONDS,
    analyze_loop_bpm,
    parse_declared_bpm,
    prepare_loop_audio,
    reconcile_quick_scan_bpm,
)
...
def normalize_rage_bpm(bpm):
    """Apply Stem Slicer's validated Rage-loop convention."""
    bpm = float(bpm)
    return bpm * 2.0 if bpm < 94.0 else bpm
...
class KeyDetectionServer:
    ...
    def __init__(self, model_path=None, device=None, num_workers=1):
        ...
        self.tempo_predictor = DeepRhythmPredictor(
            model_path=str(get_resource_path('checkpoints/deeprhythm-0.7.pth')),
            device='cpu',
            quiet=True,
        )
    ...
    def process_request(self, request, generation):
        ...
        bpm_mode = request.get('bpm_mode')
        requested_structure_ffmpeg = request.get('structure_ffmpeg_path')
        ...
        try:
            ...
            tempo_audio, tempo_sample_rate = librosa.load(
                ...
                sr=22050,
                mono=True,
                duration=LOOP_BPM_AUDIO_SECONDS
                if bpm_mode == 'quick_scan_loop'
                else 30.0,
            )
            bpm_audio = (
                prepare_loop_audio(tempo_audio)
                if bpm_mode == 'quick_scan_loop'
                else tempo_audio
            )
            raw_bpm, bpm_confidence = self.tempo_predictor.predict_from_audio(
                bpm_audio, tempo_sample_rate, include_confidence=True
            )
            normalized_bpm = normalize_rage_bpm(raw_bpm)
            bpm_details = {}
            if bpm_mode == 'quick_scan_loop':
                try:
                    bpm_details = analyze_loop_bpm(
                        audio_path,
                        bpm_audio,
                        raw_bpm,
                        ffmpeg_path=(
                            Path(requested_structure_ffmpeg)
                            if requested_structure_ffmpeg
                            else get_resource_path('ffmpeg')
                        ),
                    )
                    normalized_bpm = bpm_details['bpm']
                except Exception as exc:
                    # A malformed/very short loop must not discard a valid key
                    # result. Keep the legacy BPM as an explicit fallback and
                    # expose the reason for diagnostics and tests.
                    declared_bpm = parse_declared_bpm(audio_path.name)
                    normalized_bpm, bpm_source = reconcile_quick_scan_bpm(
                        declared_bpm, normalized_bpm
                    )
                    bpm_details = {
                        'bpm': normalized_bpm,
                        'bpm_float': float(normalize_rage_bpm(raw_bpm)),
                        'bpm_source': bpm_source,
                        'bpm_decision': 'legacy_fallback',
                        'declared_bpm': declared_bpm,
                        'bpm_refinement_error': str(exc),
                    }
            if profile:
